...
def _make_label(keywords: list[str], cid: int) -> str:
    """Pick the most descriptive keyword as a cluster label.

    Prefers bigrams (two-word phrases) over unigrams because bigrams like
    'fine tuning' or 'constitutional ai' are more informative than 'fine' or 'ai'.
    Falls back to the top-ranked unigram, then a generic name.
    """
    for kw in keywords:
        if " " in kw:
            return kw.title()
    return keywords[0].title() if keywords else f"Cluster {cid}"
...
def cluster(papers: list[Paper], n_clusters: int | None = None) -> list[Cluster]:
    """Backward-compatible free function — delegates to TfidfClusterer."""
    return TfidfClusterer().cluster(papers, n_clusters)
